...
def encrypt_to_four(plaintext):
    ...
    for c in range(0, len(plaintext), 3):
        mapping = [ord(plaintext[c]) >> 2, 3 & ord(
            plaintext[c]) << 4, None, None]
...
def cipher_keyed(key, plaintext):
    ...
    for f in range(0, len(plaintext)):
        ...
        cipher += chr(ord(plaintext[f]) ^
                      mapping[(mapping[i] + mapping[j]) % 256])
...
def encrypt(data):
    return "".join(
        BASE64_TABLE[
            int(segment.ljust(6, "0"), 2) if len(
                segment) < 6 else int(segment, 2)
        ]
        for segment in wrap(
